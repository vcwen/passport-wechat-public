...
WechatPublicStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    var agent = 'other'
    if(ua && ua.toLowerCase().indexOf('micromessenger')>=0) {agent = 'wechat'}
    var authUrlMethod = agent === 'wechat' ? 'getAuthorizeURL' : 'getAuthorizeURLForWebsite'
    var scp = agent === 'wechat' ? scope : 'snsapi_login'
    var location = this._oauth[authUrlMethod](params.redirect_uri, this._state, scp)
