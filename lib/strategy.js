...
WechatPublicStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    if(ua.toLowerCase().indexOf("micromessenger")>=0) {agent = "wechat";}
    else {scope = "snsapi_login"}
    var authUrlMethod = agent === 'wechat' ? "getAuthorizeURL" : "getAuthorizeURLForWebsite";
    var location = this._oauth[authUrlMethod](params.redirect_uri, this._state, scope);
