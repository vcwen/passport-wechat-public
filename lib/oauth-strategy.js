...
function WechatPublicStrategy(options, verify) {
  ...
  if (!options.clientID) {
    throw new TypeError('WechatPublicStrategy requires a clientID option');
  }
  if (!options.clientSecret) {
    throw new TypeError('WechatPublicStrategy requires a clientSecret option');
  }
    ...
    this._oauth.getAccessToken(code,
      function(err, result) {
        ...
        if (data.scope === 'snsapi_base') {
          verifyResult(accessToken, refreshToken,{}, {},verified);
        } else {
